    ...
    <div className="auth-page">
      ...
      <motion.div className="auth-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="auth-card__brand">
          <svg width="36" height="36" viewBox="0 0 32 32" fill="none"><rect x="2" y="2" width="28" height="28" rx="8" fill="url(#ag)"/><path d="M10 12h12M10 16h8M10 20h10" stroke="#080810" strokeWidth="2" strokeLinecap="round"/><defs><linearGradient id="ag" x1="2" y1="2" x2="30" y2="30"><stop stopColor="#00D4FF"/><stop offset="1" stopColor="#8B5CF6"/></linearGradient></defs></svg>
        ...
        <form className="auth-card__form" onSubmit={handleSubmit}>
          ...
          <div className="auth-card__field auth-card__field--pw">
            <input type={showPw ? 'text' : 'password'} className="input-glass" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required />
            <button type="button" className="auth-card__pw-toggle" onClick={() => setShowPw(!showPw)}>
              {showPw ? 'Hide' : 'Show'}
            </button>
          </div>
          <Button type="submit" variant="primary" size="lg" loading={loading} style={{ width: '100%' }}>Sign in</Button>
        </form>
